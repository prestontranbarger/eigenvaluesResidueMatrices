...
for n in range(4, 500):
    ...
    if len(vE) not in vEs:
        for element in tqdm(vE):
            ngs = conjugate(simplifyComplex(normalizedGaussSum(element) * eisensteinToComplex(extendedCubicResidueSymbol(egnNum + 0 * omega, element))))
            v.append(ngs)
        ...
        row = [parseComplex(element) for element in RWEVectors.readEVectors([rowDict[str(len(vE))]])[egnNum - 1]]
        #print(row)
